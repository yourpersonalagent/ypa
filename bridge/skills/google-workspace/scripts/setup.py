#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for YHA.
...
def _write_secret(path: Path, text: str) -> None:
    """Write text to a credentials file with owner-only (0600) perms.

    Google tokens and client secrets must never be group/world readable.
    os.open with mode 0600 creates the file restricted from the start (no
    brief 0644 window); the chmod re-applies it if the file already existed.
    On Windows mode bits are largely advisory — harmless no-op there.
    """
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same YHA skill state to refresh consent."
    )
...
def install_deps():
    ...
    try:
        ...
        print()
        print("If you hit 'externally-managed-environment' (Debian/Ubuntu), create a venv:")
        print("  python3 -m venv .venv")
        print(f"  .venv/bin/pip install {' '.join(REQUIRED_PACKAGES)}")
        print("Then use .venv/bin/python to run setup.py going forward.")
        return False
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to the YHA skill state dir."""
    ...
    _write_secret(CLIENT_SECRET_PATH, json.dumps(data, indent=2))
...
def _load_pending_auth() -> dict:
    ...
    if not data.get("state"):
        print("ERROR: Pending OAuth session is missing OAuth state.")
        print("Run --auth-url again to start a fresh OAuth session.")
        sys.exit(1)
...
def exchange_auth_code(code: str):
    ...
    _write_secret(TOKEN_PATH, json.dumps(token_payload, indent=2))
    PENDING_AUTH_PATH.unlink(missing_ok=True)
    print(f"OK: Authenticated. Token saved to {TOKEN_PATH}")
    print(f"Skill-scoped token location: {display_hermes_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for YHA")
